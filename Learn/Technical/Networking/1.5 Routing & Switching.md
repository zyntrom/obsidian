# 🔥 **1.5 Routing & Switching — FULL DETAILED EXPLANATION**

---

# 🟦 **1. Switching (Layer 2 – Ethernet)**
...
- Sends ONLY to PC2’s port
- No other device sees the traffic

➡️ This makes switches much safer than hubs.

---

## 💀 Hacker Attacks on Switches
...
➡️ Attacker becomes **Man-in-the-Middle (MitM)**.
...
Switch MAC table overflows → it enters **"hub-mode"**.

➡️ It broadcasts all traffic → attacker can sniff all packets.

---

# 🟦 **2. Routing (Layer 3 – IP)**
...
## 🧭 Routing Table Example (Explained)
...
Router checks routing table → matches the route 192.168.5.0/24  
...
# 🟦 **3. Default Gateway (VERY IMPORTANT)**
...
This IP is **not in your LAN**, so your PC sends the packet → **default gateway** → Internet.
...
➡️ **Attacker receives ALL internet-bound traffic**  
➡️ Can do:
...
# 🟦 **4. ARP (Address Resolution Protocol)—DETAILED**
...
- **NO authentication**
- Anyone can reply "I am the gateway" even if they’re not
...
➡️ Attacker sits in the middle = **MitM**
...
Attacker forces victim to downgrade from **HTTPS → HTTP**
...
```
Intel NIC MAC → A4:6B:C1:11:22:33
```
---
...
# 🟩 **Complete Example Attack Scenario**
...
```
3. Victim sends all packets → attacker
4. Attacker enables packet forwarding
```
echo 1 > /proc/sys/net/ipv4/ip_forward
```
3. Open Wireshark → captures everything
4. Optional: run **ssl-strip**
